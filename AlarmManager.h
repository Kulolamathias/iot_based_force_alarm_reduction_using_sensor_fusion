...
class AlarmManager {
  private:
    ...
    bool _buzzerOn;
    unsigned long _buzzerSilencedUntil;  // if > millis(), buzzer is muted
  ...
  public:
    AlarmManager(uint8_t buzzerPin, uint8_t redLedPin, uint8_t greenLedPin, uint8_t yellowLedPin, uint8_t relayPin, int buzzerFreq);
    void begin();
    
    void triggerWarningOnly();
    void triggerFullAlarm();
    void returnToSafe();
    void forceSafe();                     // immediate safe state (override)
    void silenceBuzzer(unsigned long durationMs);
    void update();
    
    const char* getStateString();         // for MQTT
};
